...
class TestPowerSet:
    def test_powerset(self):
        ...
        assert powerset == [[],[1],[2],[1,2],[3],[1,3],[2,3],[1,2,3]]
    ...
    def test_one_element_powerset(self):
        powerset = new_powerset([1])
        assert len(powerset) == 2**1
        assert powerset == [[],[1]]

    def test_empty_element_powerset(self):
    ...
    def test_large_powerset(self):
        s = [x for x in range(1 << 4)] # 2^4 = 16
        # 0 ~ 15 => s contains 16 elements, so the number of subsets in the powerset should be:
        # 2^16 = 65536
